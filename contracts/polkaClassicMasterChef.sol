...
library Address {
     ...
     * use https://solidity.readthedocs.io/en/latest/units-and-global-variables.htmlMhighlight=abi.decode#abi-encoding-and-decoding-functions[`abi.decode`].
     *
     * Requirements:
     *
...
contract DOTC is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    ...
    string private _name = "Polka Classic";
    string private _symbol = "DOTC";
    ...
    uint256 public _taxFee = 200; // 200.div(10000) = 2% = 0.02
    ...
    uint256 public _extraFee;
    uint256 public _maxFreeAmount;

    address public marketingWallet = 0xa46f66eAae9aB8231b72363bD2e0bEcd3445035C;
    ...
    uint256 private numTokensSellToAddToLiquidity = 5 * 10*10 * 10**_decimals;
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "PolkaClassic : The router already has that address");
    ...
    function setAntiWhale(uint256 maxFreeAmount, uint256 extraFee) external onlyOwner {
        _maxFreeAmount = maxFreeAmount.mul(10**uint256(_decimals));
        _extraFee = extraFee;
    ...
    function excludeFromReward(address account) public onlyOwner() {
        ...
        require(_excluded.length < 256, "Excluded list is too long");
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        if (_extraFee > 0 && amount >= _maxFreeAmount) {
            _previousTaxFee = _taxFee;
            _taxFee = _taxFee.add(_extraFee);
        }
    ...
    function setTaxFeePercent(uint256 taxFee, uint256 marketingFee, uint256 burnFee, uint256 liquidityFee) external onlyOwner() {
        _taxFee = taxFee;
    ...
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner() {
        require(maxTxPercent > 1000, "Cannot set transaction amount less than 10 percent!");
...
// MasterChef is the master of DOTC. He can make DOTC and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once DOTC is sufficiently
...
contract MasterChef is Ownable {
    ...
    uint256 public deductedWithdrawFee = 50; 
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of DOTCs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accDOTCPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accDOTCPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. DOTCs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that DOTCs distribution occurs.
        uint256 accDOTCPerShare;   // Accumulated DOTCs per share, times 1e12. See below.
    ...
    // DOTC tokens created per block.
    uint256 public dotcPerBlock;
    // Bonus muliplier for early dotc makers.
    ...
    // The block number when DOTC mining starts.
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        uint256 _dotcPerBlock,
        uint256 _startBlock
    ) public {
        dotc = _dotc;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        dotcPerBlock = _dotcPerBlock;
    ...
    // Update the given pool's DOTC allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending DOTCs on frontend.
    function pendingDOTC(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accDOTCPerShare = pool.accDOTCPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 dotcReward = multiplier.mul(dotcPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accDOTCPerShare = accDOTCPerShare.add(dotcReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accDOTCPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        // dotc.mint(devaddr, dotcReward.div(10));
        // dotc.mint(address(this), dotcReward);
        
         if (dotc.allowance(devaddr, address(this)) > dotcReward) {
            dotc.transferFrom(devaddr, address(this), dotcReward);
        }
        
        pool.accDOTCPerShare = pool.accDOTCPerShare.add(dotcReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for DOTC allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accDOTCPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
                safeDOTCTransfer(msg.sender, pending);
            }
        }
        if(_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accDOTCPerShare).div(1e12);
    ...
    // Safe dotc transfer function, just in case if rounding error causes pool to not have enough DOTCs.
    function safeDOTCTransfer(address _to, uint256 _amount) internal {
        uint256 dotcBal = dotc.balanceOf(address(this));
        if (_amount > dotcBal) {
            dotc.transfer(_to, dotcBal);
        } else {
            dotc.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _dotcPerBlock) public onlyOwner {
        massUpdatePools();
        dotcPerBlock = _dotcPerBlock;
    }
    ...
    function setWithdrawFeeUpdatePeriod(uint256 _second) public onlyOwner {
        WITHDRAWAL_FEE_DEDUCT_PERIOD = _second;
    }
